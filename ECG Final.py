...
lbl_titulo2 = tk.Label(master=frameparametros, bg="lemon chiffon", font=('Comic Sans MS', 15, 'bold italic'),
                       text="Parametros")
...
lbl_titulo3 = tk.Label(master=frameED, bg="lavenderblush2", font=('Comic Sans MS', 15, 'bold italic'),
                       text="Metodo de solucion ED")
...
def Cerrar():
    MsgBox = tk.messagebox.askquestion('Cerrar Aplicación', '¿Está seguro que desea cerrar la aplicación?',
                                       icon='warning')
    if MsgBox == 'yes':
        ventana.destroy()
    else:
...
def FEulerModRoot(yt2, t1, t2, y1t1, y2t1, y3t1, h, ai, bi, tetai, tiempo1, tiempo2):
    return [y1t1 + (h / 2.0) * \
            ...
            y3t1 + (h / 2.0) * \
            (difZ(y1t1, y2t1, y3t1, ai, tetai, bi, tiempo1) + difZ(yt2[0], yt2[1], yt2[2], ai, tetai, bi, tiempo2)) -
            yt2[2]]
    ...
    for i in range(1, len(tiempo)):
        ...
        solucionBack = opt.fsolve(FEulerBackRoot,
                                  np.array([XEulerBackRoot[i - 1], YEulerBackRoot[i - 1], ZEulerBackRoot[i - 1]]), (
                                      XEulerBackRoot[i - 1], YEulerBackRoot[i - 1], ZEulerBackRoot[i - 1], T, tiempo, h,
                                      ai,
                                      tetai, bi), xtol=10 ** -5)
...
def RK2():
    ...
    for i in range(1, len(tiempo)):
        ...
        k32 = difZ(XRK2[i - 1] + h * k11, YRK2[i - 1] + h * k21, ZRK2[i - 1] + h * k31, ai, tetai, bi,
                   tiempo[i - 1] + h)
...
def amplitudIPM(fc):
    ...
    if (fc <= 60):
        ...
        peaks, properties = sc.find_peaks(ecg, height=0.02,
                                          width=1)  # Estas restricciones del detector de picos se hace con
...
def exportarDatos():
    ...
    empaque2 = struct.pack("d" * len(EulerAdelante()),
                           *EulerAdelante())  # Se empaqueta el eje Z el cual tiene el valor de las ondas PQRST con tamaño "d"
...
def importarDatos():
    ...
    desempaqueX = struct.unpack("d" * int(len(lecturaX) / 8),
                                lecturaX)  # Para leer correctamente los archivos se debe desempacar con el tamano double "8"
    desempaqueY = struct.unpack("d" * int(len(lecturaY) / 8), lecturaY)

    fllX.close()
    fllY.close()

    plt.plot(desempaqueX,
             desempaqueY)  # Se mostrará cuando el usuario desee importar lo datos(es decir que debe clickear el botón "importar"
...
BotoneulerA = ttk.Button(master=frameED, text="Euler adelante", style="1.TButton", command=GraficaEulerFor,
                         width=13).place(x=160, y=68)
Botoneuleratras = ttk.Button(master=frameED, text="Euler atras", style="1.TButton", command=GraficaEulerBack,
                             width=13).place(x=160, y=105)
Botoneulermod = ttk.Button(master=frameED, text="Euler modificado", style="1.TButton", command=GraficaEulerModificado,
                           width=13).place(x=160, y=142)
BotonRK2 = ttk.Button(master=frameED, text="Runge-Kutta2", style="1.TButton", command=GraficaRK2, width=13).place(x=160,
                                                                                                                  y=179)
BotonRK4 = ttk.Button(master=frameED, text="Runge-Kutta4", style="1.TButton", command=GraficaRK4, width=13).place(x=160,
                                                                                                                  y=216)

BotonhallarHR = ttk.Button(master=ventana, text="Hallar HR", style="3.TButton", command=MensajeH, width=15).place(x=100,
                                                                                                                  y=340)

# botones de exportar y cargar datos
BotonexportarD = ttk.Button(master=ventana, text="Exportar datos", style="3.TButton", command=exportarInterface,
                            width=20).place(x=60, y=10)
Botoncargardatos = ttk.Button(master=ventana, text="Cargar datos", style="3.TButton", command=importarInterface,
                              width=20).place(x=220, y=10)

# Boton que cambia
gris1 = Image.open("gris.png")
gris1 = gris1.resize((20, 20), Image.LANCZOS)
